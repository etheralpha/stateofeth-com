...
from consensus_client_diversity_validators.migalabs import migalabs_marketshare
...
def get_rated_overview_data():
  if utilities.use_test_data:
    ...
    headers = {
      ...
      'Authorization': utilities.rated_token
    }
    response = utilities.fetch_json(url, "GET", payload, headers)
    return response

def process_rated_overview_data(raw_data):
  # get the validator count
  validator_count = raw_data["data"][0]["validatorCount"]
  utilities.print_data("processed", validator_count, "validator_count")
...
def process_rated_operator_pool_data(raw_data):
  ...
  utilities.print_data("processed", pool_validator_counts, "pool_validator_counts")
...
def get_edi_marketshare_data():
  if utilities.use_test_data:
    ...
    response = utilities.fetch_json(url)
    return response
...
def process_edi_marketshare_data(raw_data, pool_validator_counts, total_validator_count):
  ...
  # utilities.pprint(["cleaned_data", cleaned_data])
  ...
  # utilities.pprint(["calculated_data", calculated_data])
  # utilities.pprint(["validators_represented", validators_represented])
  ...
  # utilities.pprint(["extra_data", extra_data])

  # create final data dict
  final_data["distribution"] = sorted_data
  final_data["other"] = extra_data
  utilities.print_data("processed", final_data, "final_data_edi")
...
def process_rated_marketshare_data(raw_data):
  ...
  final_data["distribution"] = sorted_data
  final_data["other"] = extra_data
  utilities.print_data("processed", final_data, "final_data_rated")

  return final_data

def rated_marketshare():
  raw_data = get_rated_marketshare_data()
  utilities.save_to_file("../_data/raw/rated_raw.json", raw_data)
  processed_data = process_rated_marketshare_data(raw_data)
  utilities.save_to_file("../_data/rated.json", processed_data)
...
def process_blockprint_accuracy_data(raw_data):
  ...
  utilities.print_data("processed", accuracy_data, "blockprint_accuracy_data")
...
def process_blockprint_marketshare_data(raw_marketshare_data, processed_accuracy_data):
  ...
  final_data["other"] = extra_data
  utilities.print_data("processed", final_data, "final_data_blockprint")
...
def blockprint_marketshare():
  ...
  utilities.save_to_file("../_data/raw/blockprint_raw.json", raw_marketshare_data)
  raw_accuracy_data = get_blockprint_accuracy_data()
  utilities.save_to_file("../_data/raw/blockprint_accuracy_raw.json", raw_accuracy_data)
  ...
  utilities.save_to_file("../_data/blockprint.json", processed_marketshare_data)
...
def process_ethernodes_marketshare_data(raw_data):
  ...
  final_data["distribution"] = sorted_data
  final_data["other"] = extra_data
  utilities.print_data("processed", final_data, "final_data_ethernodes")

  return final_data

def ethernodes_marketshare():
  raw_data = get_ethernodes_marketshare_data()
  utilities.save_to_file("../_data/raw/ethernodes_raw.json", raw_data)
  processed_data = process_ethernodes_marketshare_data(raw_data)
  utilities.save_to_file("../_data/ethernodes.json", processed_data)
